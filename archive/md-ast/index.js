...
const parse = (input, top = []) => {
  ...
  while(true) {
    ...
    // Code blocks: (-1 to +1)
      ...
      const syntaxParts = syntax.trim().split(" ");
      if(syntaxParts.length > 1) {
        syntax = syntaxParts[0];
        meta = syntaxParts.slice(1).reduce((metaObj, part) => {
          const metaPart = part.split("=");
          if(metaPart.length > 1) {
            metaObj[metaPart[0]] = metaPart[1];
          } else {
      ...
      const metaString = syntaxParts.slice(1).join(' ')
      addNode({
        type: "codeBlock",
        code,
        syntax,
    ...
    // Inline formatting (-0 to +0)
      ...
      let style = match[i];
      let type =
        style.length < 2 ? "italic" : style == "~~" ? "strike" : "bold";
